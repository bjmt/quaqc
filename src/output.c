...
int append_json_result(char *fn, results_t *results, const params_t *params) {
  ...
  fjson_write(fjson, "               \"seq_n\": %"PRId64",\n", results->seq_n);
  fjson_write(fjson, "               \"seq_size\": %"PRId64"\n", results->seq_sum);
  fjson_write(fjson, "            },\n");
  ...
  fjson_write(fjson, "          \"total\": {\n");
  fjson_write(fjson, "            \"reads\": %"PRId64",\n", results->r_total);
  fjson_write(fjson, "            \"unmapped\": %"PRId64",\n", results->r_unmapped);
  fjson_write(fjson, "            \"mapped\": %"PRId64",\n", results->r_mapped);
  fjson_write(fjson, "            \"blacklisted\": %"PRId64"\n", results->r_mapped_bl);
  fjson_write(fjson, "          },\n");
  fjson_write(fjson, "          \"effective\": {\n");
  fjson_write(fjson, "            \"mapped_n\": %"PRId64",\n", results->r_seen);
  fjson_write(fjson, "            \"nuclear\": {\n");
  fjson_write(fjson, "              \"n\": %"PRId64",\n", null_get(results->nucl, reads_n));
  fjson_write(fjson, "              \"duplicated\": %"PRId64",\n", null_get(results->nucl, dups_n));
  fjson_write(fjson, "              \"single_end\": %"PRId64",\n", null_get(results->nucl, se_n));
  fjson_write(fjson, "              \"paired_end\": %"PRId64",\n", null_get(results->nucl, pe_n));
  fjson_write(fjson, "              \"properly_mated\": %"PRId64",\n", null_get(results->nucl, mated_n));
  fjson_write(fjson, "              \"primary_alignments\": %"PRId64",\n", null_get(results->nucl, pri_n));
  fjson_write(fjson, "              \"duplicated_primary_alignments\": %"PRId64",\n", null_get(results->nucl, dups_pri_n));
  ...
  fjson_write(fjson, "              \"secondary_alignments\": %"PRId64",\n", null_get(results->nucl, sec_n));
  fjson_write(fjson, "              \"supplementary_alignments\": %"PRId64"\n", null_get(results->nucl, sup_n));
  fjson_write(fjson, "            },\n");
  fjson_write(fjson, "            \"mitochondrial\": {\n");
  ...
  if (params->omit_depth) { // TODO: I could still print LN_G if I wanted to...
    ...
    fjson_write(fjson, "              \"addn_stats_are_suspect\": %s,\n", strbool(warn_depth));
    fjson_write(fjson, "              \"depths_min\": %"PRId64",\n", null_get(results->nucl, min_depth));
    fjson_write(fjson, "              \"depths_1st_pctile\": %"PRId64",\n", null_get(results->nucl, pct1_depth));
...
#define printWS(N, CON) do { for (int i = 0; i < N; i++) fputc(' ', CON); }  while(0)
...
int print_results(char *fn, results_t *results, const params_t *params) {
  ...
  printWS(WS, fout);
  fprintf(fout, "Minimum alignment size:     %'10"PRId64"\n", params->qlen_min);
  printWS(WS, fout);
  fprintf(fout, "Maximum alignment size:     %'10"PRId64"\n", params->qlen_max);
  printWS(WS, fout);
  fprintf(fout, "Minimum fragment size:      %'10"PRId64"\n", params->flen_min);
  printWS(WS, fout);
  fprintf(fout, "Maximum fragment size:      %'10"PRId64"\n", params->flen_max);
  ...
  printWS(WS, fout);
  fprintf(fout, "%s", "\u251C\u252C\u2500");
  fprintf(fout, " Plastid:                    %'14"PRId64" (%'.1f%%)\n", pltd_n, pltd_n_pct);
  ...
  printWS(WS, fout);
  fprintf(fout, "%s", "\u2514\u2500\u2500");
  fprintf(fout, " Supplementary alignments:   %'14"PRId64" (%'.1f%%)\n", sup_total, sup_pct);
  fputc('\n', fout);

  print_in_sbox("Nuclear read stats after filtering", 60, fout);
  fputc('\n', fout);

  double filt_pct = calc_pct(null_get(results->nucl, filt_n), results->r_seen);
  printWS(WS, fout);
  fprintf(fout, "Reads passing filters:          %'14"PRId64" (%'.1f%%)\n", null_get(results->nucl, filt_n), filt_pct);
  fputc('\n', fout);
  bool warn_qlen = params->qhist_max < params->qlen_max && !!results->nucl_shared->read_sizes[params->qhist_max];
  printWS(WS, fout);
  ...
  if (warn_qlen) {
    ...
    printWS(WS, fout);
    fprintf(fout, "%s", "\u2500\u2500\u25Ba");
    fprintf(fout, " Max filter size:            %'14"PRId64" (--max-qlen)\n", params->qlen_max);
    printWS(WS, fout);
    fprintf(fout, "%s", "\u252C\u2500\u25Ba");
    fprintf(fout, " Max histogram size:         %'14d (--max-qhist)\n", params->qhist_max);
    printWS(WS, fout);
    fprintf(fout, "%s", "\u2514\u2500\u2500\u25Ba");
    fprintf(fout, " Max recorded size:         %'14"PRId64"\n", null_get(results->nucl, max_qlen));
    fputc('\n', fout);
  }
  printWS(WS, fout);
  fprintf(fout, "Fragments passing filters:      %'14"PRId64"\n", null_get(results->nucl, frags_n));
  fputc('\n', fout);
  bool warn_flen = params->fhist_max < params->flen_max && !!results->nucl_shared->frag_sizes[params->fhist_max];
  printWS(WS, fout);
  ...
  if (warn_flen) {
    ...
    printWS(WS, fout);
    fprintf(fout, "%s", "\u2500\u2500\u25Ba");
    fprintf(fout, " Max filter size:            %'14"PRId64" (--max-flen)\n", params->flen_max);
    printWS(WS, fout);
    fprintf(fout, "%s", "\u252C\u2500\u25Ba");
  ...
  if (params->omit_depth) {
    ...
    if (warn_depth) {
      ...
      printWS(WS, fout);
      fprintf(fout, "%s", "\u2514\u2500\u2500\u25Ba");
      fprintf(fout, " Max recorded depth:        %'14"PRId64"\n", null_get(results->nucl, max_depth));
    }
  }
  fputc('\n', fout);
  ...
  printWS(WS, fout);
  fprintf(fout, "%s", "\u251C\u2500\u2500");
  fprintf(fout, " Min:                        %'14"PRId64"\n", null_get(results->nucl, min_mapq));
  printWS(WS, fout);
  fprintf(fout, "%s", "\u251C\u2500\u2500");
  fprintf(fout, " 1st pctile:                 %'14"PRId64"\n", null_get(results->nucl, pct1_mapq));
  printWS(WS, fout);
  fprintf(fout, "%s", "\u251C\u2500\u2500");
  fprintf(fout, " Average:                    %'18.3f\n", null_get(results->nucl, avg_mapq2));
  printWS(WS, fout);
  ...
  printWS(WS, fout);
  fprintf(fout, "%s", "\u251C\u2500\u2500");
  fprintf(fout, " 99th pctile:                %'14"PRId64"\n", null_get(results->nucl, pct99_mapq));
  printWS(WS, fout);
  fprintf(fout, "%s", "\u251C\u2500\u2500");
  fprintf(fout, " Max:                        %'14"PRId64"\n", null_get(results->nucl, max_mapq));
  printWS(WS, fout);
  fprintf(fout, "%s", "\u2514\u2500\u2500");
  ...
  fputc('\n', fout);
  printWS(WS, fout);
  fprintf(fout, "Note: Min/max and percentile stats are always printed\n");
  printWS(WS, fout);
  fprintf(fout, "      as integers.\n");
  fputc('\n', fout);
